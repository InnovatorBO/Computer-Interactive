<script lang="ts">
  ...
  let container: HTMLElement;  // Reference to the div containing the canvas
  ...
  onMount(() => {
    ...
    const clickableObjects: THREE.Mesh[] = []; // Store objects that can be clicked
    ...
    function onClick(event: MouseEvent | Touch) {
      if (
      ...
      if (intersects.length > 0) {
        ...
        // Find info for clicked object
        const objectName = clickedObject.name;
        let foundInfo = null;

        for (const infoItem of info) {
          if (infoItem.components.includes(objectName)) {
            foundInfo = infoItem;
            break;
          }
        }
        ...
        if (foundInfo) {
          infoText = foundInfo.text;
          showInfo = true;
          infoLeft = event.clientX;
          infoTop = event.clientY;
        } else if (objectName === "pCube1171" || objectName === "pCube1169" || objectName === "pCube1000") {
          infoText = gpuText;
          showInfo = true;
          infoLeft = event.clientX;
          infoTop = event.clientY;
        } else if (objectName === "pCube511" || objectName === "pCube476") {
  ...
  onDestroy(() => {
    ...
    for (const handler of toDestroy) {
